...
# print(count_questions(group_questions))
    ...
    for y in first:
      for z in rest: 
        if y in z:
          count += 1
          new_array.append(y)
  new_dict = Counter(new_array)
  for key in new_dict:
      if new_dict[key] == len(rest):
        total_count += 1
  return total_count
...
def sort_strings(array):
  ...
  for x in array:
    if len(x) == 1:
      new_array = list(dict.fromkeys(x[0]))
      total_count += len(new_array)
    new_array = []
    for y in x:
      sorted_y = ''.join(sorted(y))
      new_array.append(sorted_y)
      new_array.sort(key=lambda s: len(s))
    total_count += find_chars(new_array)

  return total_count

print(sort_strings(group_questions))